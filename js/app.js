...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image img-fluid" src=${product.image}></img>
      ...
      <h3 class = "titles">${product.title}</h3>
      <p>Category: ${product.category}</p>
      <h2 class = "price">Price: $ ${product.price}</h2>
      <div class = "rating"> <p>rating: <span class = "rate">${product.rating.rate}(${product.rating.count})</span></p>  </div>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" 
      class="buy-now btn add-cart">add to cart</button>
...
const details = (details) =>{
  fetch(`https://fakestoreapi.com/products/${details}`)
            .then(res=>res.json())
...
box.textContent = ""
 const div  = document.createElement("div")
 div.innerHTML = `
 <h4><span class = "title"> ${data.title.slice(0,37)}</span></h4>
 <h5 class = "product-details"> product details:</h5>
 <p class = "details"> ${data.description.slice(0,100)}</p>
...
const updateTotal = () => {
 ...
 const tax = document.getElementById("total-tax").innerText
  const grandTotal = parseFloat(price) + parseFloat(delivery) + parseFloat(tax)
